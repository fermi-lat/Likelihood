...
def test_freeze_thaw_weighted(dirs):
    ...
    myconf = BinnedConfig(edisp_bins=dispbins,save_all_srcmaps=False, no_cached_weightmaps = True) # modified
    obs = BinnedObs(irfs=irfs, expCube=ltcube, srcMaps=srcmap, binnedExpMap=bexpmap)
    like = BinnedAnalysis(obs, srcmodel, optimizer="Minuit", config=myconf, wmap = wmap) #modified
