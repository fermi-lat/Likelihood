...
void BinnedLikelihood::buildFixedModelWts(bool process_all) {
    ...
    // std::cout << "BinnedLikelihood::buildFixedModelWts(): m_fixed_counts_spec.size() = " << m_fixed_counts_spec.size()
    //           << ", m_fixedSources.size() = " << m_fixedSources.size() << std::endl;
        ...
        if (src->fixedSpectrum()) {
            ...
            if (srcMap == 0) {
                throw std::runtime_error(
                    "BinnedLikelihood::fixedModelUpdated: "
                    "inconsistent m_srcMaps.");
            }
            if (srcMap->spectrum_changed()) {
                changed = true;
...
// void BinnedLikelihood::buildFixedModelWts(bool process_all) {
//   std::cout << "BinnedLikelihood::buildFixedModelWts(" << process_all << "): m_fixed_counts_spec.size() = " << m_fixed_counts_spec.size()
//             << ", m_fixedSources.size() = " << m_fixedSources.size() << std::endl;
//   if (m_fixed_counts_spec.size() == 0 && m_fixedSources.size() != 0 ) {
...
//           if ( ! process_all ) {
//             // std::cout << "BinnedLikelihood::buildFixedModelWts() calling addFixedSource()" << std::endl;
//             addFixedSource(srcName);
//           } else {
//             m_srcMapCache.getSourceMap(*src, false);
//           }
...
void BinnedLikelihood::deleteFixedSource(const std::string& srcName) {
    ...
    std::vector<std::string>::iterator it = std::find(m_fixedSources.begin(), m_fixedSources.end(), srcName);
...
int BinnedLikelihood::edisp_val(const std::string& srcname) const {
    ...
    if (srcname != "") {
        ...
        /// background models.
        const std::map<std::string, Source*>::const_iterator& it(m_sources.find(srcname));
        if (it != m_sources.end()) {
...
void BinnedLikelihood::setImageDimensions(tip::Image* image,
    ...
    if (3 != num_dims) {
        throw std::runtime_error("BinnedLikelihood::setImageDimensions: " + std::string("Cannot write a SourceMap file ") + "to an image which is not 3D");
    }
...
void BinnedLikelihood::addSourceCounts(std::vector<double>& modelCounts,
    ...
    if (modelCounts.size() != m_dataCache.nFilled()) {
        throw std::runtime_error(
            "BinnedLikelihood::addSourceCounts: "
